class RefreshAllNorwegianPricesJob < ApplicationJob
  ...
  def perform
    ...
    flight_searches.find_each do |search|
      ...
      if result[:success]
        ...
        failures << {
          ...
          destination: search.norwegian_destination&.code,
          dates: "#{search.date_out.strftime('%d %b')} - #{search.date_in.strftime('%d %b %Y')}",
          error: result[:error] || "Unknown error"
        }
        Rails.logger.error "[RefreshAllNorwegianPricesJob] Failed to fetch price for search ##{search.id}: #{result[:error]}"
      end
    end
    ...
    if failures.any?
      Rails.logger.warn "[RefreshAllNorwegianPricesJob] Sending failure notification for #{failures.count} failed fetches"
      FetchFailureMailer.fetch_failed(airline: "Norwegian", failures: failures).deliver_later
    end

    Rails.logger.info "[RefreshAllNorwegianPricesJob] Completed price refresh. Sent notifications to #{price_drops_by_user.keys.count} users. Failures: #{failures.count}"
  end
end

class RefreshAllBodePricesJob < ApplicationJob
  queue_as :default
  ...
  def perform
    ...
    flight_searches.find_each do |search|
      ...
      if search.date_out < Date.current
        Rails.logger.info "[RefreshAllBodePricesJob] Skipping flight search ##{search.id} - departure date is in the past"
        next
      end
      ...
      if result[:success]
        if result[:price_drop].present?
          ...
          if result[:price_drop][:savings] >= user.price_notification_threshold
            ...
            Rails.logger.info "[RefreshAllBodePricesJob] Price drop detected for user #{user.id}: #{result[:price_drop][:savings]}€"
          else
            Rails.logger.info "[RefreshAllBodePricesJob] Price drop of #{result[:price_drop][:savings]}€ below threshold for user #{user.id}"
          end
        end
      else
        ...
        failures << {
          ...
          destination: search.bode_destination&.code,
          dates: "#{search.date_out.strftime('%d %b')} - #{search.date_in.strftime('%d %b %Y')}",
          error: result[:error] || "Unknown error"
        }
        Rails.logger.error "[RefreshAllBodePricesJob] Failed to fetch price for search ##{search.id}: #{result[:error]}"
      end
    end
    ...
    price_drops_by_user.each do |user_id, price_drops|
      ...
      Rails.logger.info "[RefreshAllBodePricesJob] Sending price drop notification to #{user.email} with #{price_drops.count} drops"
    ...
    if failures.any?
      Rails.logger.warn "[RefreshAllBodePricesJob] Sending failure notification for #{failures.count} failed fetches"
      FetchFailureMailer.fetch_failed(airline: "Bode.lv", failures: failures).deliver_later
    end

    Rails.logger.info "[RefreshAllBodePricesJob] Completed price refresh. Sent notifications to #{price_drops_by_user.keys.count} users. Failures: #{failures.count}"
  end
end

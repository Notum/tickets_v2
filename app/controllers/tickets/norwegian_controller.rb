module Tickets
  class NorwegianController < ApplicationController
    def index
      @destinations = NorwegianDestination.active.ordered
      @saved_searches = current_user.norwegian_flight_searches.includes(:norwegian_destination).recent
      @selected_destination_code = params[:destination_code] if params[:destination_code].present?
    end

    def create
      destination = NorwegianDestination.find_by(code: params[:destination_code])

      unless destination
        flash[:alert] = "Destination not found"
        redirect_to tickets_norwegian_path and return
      end
      ...
      @flight_search = current_user.norwegian_flight_searches.build(
        norwegian_destination: destination,
      ...
      if @flight_search.save
        ...
        if @flight_search.price_out.present? && @flight_search.price_in.present? &&
          ...
          FetchNorwegianPriceJob.perform_later(@flight_search.id)
          flash[:notice] = "Flight search saved! Price will be fetched in the background."
        end

        redirect_to tickets_norwegian_path(destination_code: destination.code)
      else
    ...
    def destroy
      @flight_search = current_user.norwegian_flight_searches.find_by(id: params[:id])
      ...
      if @flight_search&.destroy
        respond_to do |format|
          ...
          format.html do
            ...
            redirect_to tickets_norwegian_path
          end
        end
      else
        respond_to do |format|

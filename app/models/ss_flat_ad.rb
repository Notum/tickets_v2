class SsFlatAd < ApplicationRecord
  DEAL_TYPES = %w[sell buy rent_out rent_want exchange].freeze
  BUILDING_SERIES = %w[103 104 119 467 602 hruscevka spec. proj. lt. proj. stalinkas renov.].freeze

  belongs_to :ss_region
  belongs_to :ss_city, optional: true
  has_many :ss_flat_follows, dependent: :destroy
  has_many :followers, through: :ss_flat_follows, source: :user
  has_many :price_histories, class_name: "SsFlatPriceHistory", dependent: :destroy
  ...
  def display_title
    title.presence || "#{rooms}-room flat in #{street.presence || ss_city&.name_lv || ss_region.name_lv}"
  end
  ...
  def floor_display
    return nil unless floor_current
    floor_total ? "#{floor_current}/#{floor_total}" : floor_current.to_s
  end
  ...
  def should_regenerate_hash?
    new_record? || ss_region_id_changed? || ss_city_id_changed? ||
      ...
      floor_current_changed? || floor_total_changed? || building_series_changed?
  end
end

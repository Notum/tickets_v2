class TurkishFlightSearch < ApplicationRecord
  belongs_to :user
  has_many :price_histories, class_name: "TurkishPriceHistory", dependent: :destroy
  ...
  def direct_flight?
    # Turkish Airlines flights via Istanbul are never direct
    false
  end

  def route_display
    "#{ORIGIN_CODE} - IST - #{destination_code}"
  end
